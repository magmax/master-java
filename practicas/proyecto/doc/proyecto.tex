...
\begin{figure}
  ...
  \caption{Ventana de facturación antes de la búsqueda}
...
\begin{figure}
  ...
  \caption{Ventana de facturación después de la búsqueda}
...
El proyecto se ha dividido en 4 partes que se desglosarán a
continuación.

\subsection{Capa de persistencia}

Dado que la capa de persistencia era común a la interfaz
administrativa y web, se decidió utilizar un \textit{JAR} separado.

Este subproyecto utiliza Hibernate sobre JPA para acceder a la
persistencia.

Contiene pruebas unitarias; para que se ejecuten rápidamente, se
utiliza una base de datos de tipo HyperSQL en memoria.

\subsection{Tabla CRUD}

Se identificó un problema común que iba a replicarse numerosas veces:
la creación de una tabla con las operaciones CRUD (Create, Retrieve,
Update, Delete). Por ello se decidió crear un proyecto a parte, con el
fin de utilizarlo posteriormente en futuros proyectos. Éste es un
proyecto que tengo intención de continuar, aunque es posible que tenga
que comenzarlo de nuevo.

El uso de genéricos en esta parte es importante. Se trató de crear una
arquitectura sencilla, pero no fue ése el resultado, complicándose un
poco cuando se metió la persistencia.
...
\item {CrudTable} Es la vista. Contiene la tabla y cuatro botones,
  pero éstos solo se dibujarán si se dispone de acciones para ellos.
\item {Acciones}, divididas en 4: CrudUICreate, CrudUIUpdate,
  CrudUIDetails y CrudUIDelete. Esta última no suele ser neceario
  implementarla porque se ofrece una por defecto. Representan el
  controlador. Pueden implementarse en una única clase o en clases
  separadas.
\end{itemize}

La idea es la siguiente: tras crear una tabla utilizando el modelo, se
le asignan las acciones que serán capaces de realizar. De esta manera,
la tabla sabe qué elementos tiene que pintar.

Para facilitar las operaciones, se han proporcionado elementos por
defecto que facilitan la creación de elementos.

Veamos un ejemplo de creación de una tabla sencilla (sacado de la
...
tabla y, posteriormente, se añaden las acciones.

Para ver el ejemplo completo, se recomienda ver el código de la
herramienta de administración. La apariencia es la que se muestra en
...
Hace uso intensivo de las tablas CRUD. Todo lo demás se ha explicado ya.

\subsection{Parte web}

Las tecnologías utilizadas han sido: \textbf{Struts} para la vista y
el controlador; \textbf{JQuery} para los efectos visuales y el estilo CSS.

Todo lo demás quedó explicado en el tema del manual de usuario.
